...
Config = {
	['DefaultIcon1'] = 'fa-solid fa-bars', -- https://fontawesome.com/search
	['DefaultIcon2'] = 'fa-solid fa-right-to-bracket', -- https://fontawesome.com/search
	['NoVehicleAlert'] = 'Vehicles are not allowed',
	['BlipsEnabled'] = false,
	["Key"] = "E"
}
...
Config['Teleports'] = {
    ...
    [85] = {['text'] = 'Enter Nightclub', ['dest'] = {86, 87}, ['coord'] = vector3(333.28, -997.71, 29.12), ['h'] = 186.64},
    ...
    [199] = {['text'] = 'Offices', ['coord'] = vector3(2517.6, -261.34, -55.12), ['h'] = 353.5, ['dest'] = {245, 197, 198, 200, 201, 202, 203}, ['veh'] = false},
    ...
    --       Arena Wars
    --[206] = {['text'] = 'Exit Arena', ['coord'] = vector3(-283.5996, -1920.6, 29.9), ['h'] = 0.0, ['dest'] = {207}},
    --[207] = {['text'] = 'Enter Arena', ['coord'] = vector3(2834.96, -3918.0, 140.0), ['h'] = 0.0, ['dest'] = {206}},
...
CreateThread(function()
        ...
        for i=1,#Config['Teleports'] do
			if Config['Teleports'][i] then
				zones[#zones+1] = BoxZone:Create(
				Config['Teleports'][i]['coord'], 1, 1, {
				name = Config['Teleports'][i]['text'],
				minZ = Config['Teleports'][i]['coord']['z'] - 2,
				maxZ = Config['Teleports'][i]['coord']['z'] + 2,
				debugPoly = false,
            })
			end
        end

        local teleportzones = ComboZone:Create(zones, {name = 'teleportzones', debugPoly = false})
        teleportzones:onPlayerInOut(function(isPointInside, _, zone)
		if isPointInside then
			isnext = true
			exports['qb-core']:DrawText('[E] - '..zone.name, 'left')
		else
			isnext = false
...
RegisterNetEvent('Teleport', function(args)
	...
	if (vehicle ~= 0) then
		entity = vehicle
		if (Config['Teleports'][id]['veh'] == false) then
			TriggerEvent('QBCore:Notify', (Config['NoVehicleAlert']), 'error')
			return
		end
	end
	
	DoScreenFadeOut(500)
	SetEntityCoordsNoOffset(entity, Config['Teleports'][id]['coord'], false, false, false)
	while not HasCollisionLoadedAroundEntity(entity) do
        FreezeEntityPosition(entity, true)
        SetEntityCoords(entity, Config['Teleports'][id]['coord'], 1, 0, 0, 1)
        RequestCollisionAtCoord(Config['Teleports'][id]['coord'])
        Wait(500)
    end
	Wait(500)
	...
	Wait(500)
	SetEntityCoordsNoOffset(entity, Config['Teleports'][id]['coord'], false, false, false)
	SetGameplayCamRelativeHeading(Config['Teleports'][id]['h'])
	SetGameplayCamRelativePitch(-20.0, 1.0)
	SetEntityHeading(entity, Config['Teleports'][id]['h'])
	Wait(500)
...
RegisterCommand('+teleport', function()
	if not isnext then return end
	local player = PlayerPedId()
    local playerCoords = GetEntityCoords(player)
    for i=1,#Config['Teleports'] do
		if Config['Teleports'][i] then
			if #(Config['Teleports'][i]['coord'] - playerCoords) <= 2 then
				if #Config['Teleports'][i]['dest'] > 1 then TriggerEvent('qbmenu',i) return end
					TriggerEvent('Teleport',Config['Teleports'][i]['dest'][1])
				break
			end
		end
	end
...
RegisterKeyMapping('+teleport', 'Teleport', 'keyboard', Config.Key)

RegisterNetEvent('qbmenu', function(id)
	local menu = {}
    menu[#menu + 1] = {
        isMenuHeader = true,
        header = Config['Teleports'][id]['text'],
        icon = Config['DefaultIcon1']
    }
    for _,dest in pairs(Config['Teleports'][id]['dest']) do
        menu[#menu + 1] = {
            header = Config['Teleports'][dest]['text'],
            txt = Config['Teleports'][dest]['text'],
            icon = Config['DefaultIcon2'],
            params = {
				event = 'Teleport',
				args = {
					id = id,
